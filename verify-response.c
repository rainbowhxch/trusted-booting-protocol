...
VerifyResponseReturnCode VerifyResponse_empty_new(VerifyResponse **verify_response) {
...
VerifyResponseReturnCode VerifyResponse_new(const VerifyResponseItem nonce, const VerifyResult verify_result, VerifyResponse **verify_response) {
	...
	CryptoReturnCode crc = CryptoMsg_new(nonce->data, nonce->data_len, &(*verify_response)->nonce);
	CRYPTO_GOTO_IF_ERROR(crc);
	crc = CryptoMsg_new((unsigned char *)(&verify_result), sizeof(verify_result), &(*verify_response)->verify_result);
	CRYPTO_GOTO_IF_ERROR(crc);

	if ((vrc = VerifyResponse_sign(*verify_response)) != VERIFY_RESPONSE_RC_SUCCESS) {
...
VerifyResponseReturnCode VerifyResponse_sign(VerifyResponse *verify_response) {
	CryptoMsgDataLength need_sign_msg_len = verify_response->nonce->data_len + verify_response->verify_result->data_len;
	...
	memcpy(off, verify_response->verify_result->data, verify_response->verify_result->data_len);

	crc = Crypto_rsa_file_digest_sign(need_sign_msg, kRSA_PRI_FILE_PATH, &verify_response->signature);
	CRYPTO_GOTO_IF_ERROR(crc);
...
VerifyResponseReturnCode VerifyResponse_verify(const VerifyResponse *verify_response, int *verify_res) {
	CryptoMsgDataLength need_verify_msg_len = verify_response->nonce->data_len + verify_response->verify_result->data_len;
	...
	crc = Crypto_rsa_file_digest_verify(need_verify_msg, verify_response->signature, kRSA_PUB_FILE_PATH, verify_res);
	CRYPTO_GOTO_IF_ERROR(crc);
...
VerifyResponseReturnCode VerifyResponse_to_json(const VerifyResponse *verify_response, char **verify_response_josn) {
	...
	CryptoReturnCode crc = CryptoMsg_to_hexstr(verify_response->nonce, &nonce_str);
	CRYPTO_GOTO_IF_ERROR(crc);
...
VerifyResponseReturnCode VerifyResponse_parse_from_json(const char *verify_response_josn, VerifyResponse **verify_response) {
	VerifyResponseReturnCode vrc = VerifyResponse_empty_new(verify_response);
	VERIFY_RESPONSE_RETURN_IF_ERROR(vrc);
	cJSON *root = cJSON_Parse(verify_response_josn);
	cJSON *nonce = cJSON_GetObjectItemCaseSensitive(root, "nonce");
	cJSON *verify_result = cJSON_GetObjectItemCaseSensitive(root, "verify_result");
	cJSON *signature = cJSON_GetObjectItemCaseSensitive(root, "signature");

	CryptoReturnCode crc = CryptoMsg_parse_from_hexstr(cJSON_GetStringValue(nonce), &(*verify_response)->nonce);
	CRYPTO_GOTO_IF_ERROR(crc);
	crc = CryptoMsg_parse_from_hexstr(cJSON_GetStringValue(verify_result), &(*verify_response)->verify_result);
	CRYPTO_GOTO_IF_ERROR(crc);
	crc = CryptoMsg_parse_from_hexstr(cJSON_GetStringValue(signature), &(*verify_response)->signature);
	CRYPTO_GOTO_IF_ERROR(crc);
