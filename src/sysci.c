...
SysciReturnCode Sysci_encrypt(const Sysci *sysci, CryptoMsg **encrypted_sysci) {
	...
	CRYPTO_GOTO_IF_ERROR(crc);
	crc = Crypto_rsa_pub_file_encrypt(sysci->sdw_tpm_sha256, kRSA_PUB_FILE_PATH, &encrypted_sdw_tpm_sha256);
	CRYPTO_GOTO_IF_ERROR(crc);
	...
	CryptoMsgDataLength encrypted_sysci_len = encrypted_hardware_id->data_len + encrypted_system_release->data_len + \
								 encrypted_efi_sha256->data_len + encrypted_sdw_tpm_sha256->data_len + encrypted_proxy_p_sha256->data_len;
	...
	memcpy(off, encrypted_sdw_tpm_sha256->data, encrypted_sdw_tpm_sha256->data_len);
...
SysciReturnCode Sysci_decrypt(const CryptoMsg *encrypted_sysci, Sysci **sysci) {
	...
	CryptoMsg_free(encrypted_efi_sha256);
	Crypto_rsa_pri_file_decrypt(encrypted_sdw_tpm_sha256, kRSA_PRI_FILE_PATH, &(*sysci)->sdw_tpm_sha256);
	CryptoMsg_free(encrypted_sdw_tpm_sha256);
...
SysciReturnCode Sysci_parse_from_json(const char *sysci_json, Sysci **sysci) {
	...
	CRYPTO_GOTO_IF_ERROR(crc);
	crc = CryptoMsg_parse_from_hexstr(cJSON_GetStringValue(sdw_tpm_sha256), &(*sysci)->sdw_tpm_sha256);
	CRYPTO_GOTO_IF_ERROR(crc);
