...
#define SYSCI_PRINT_CRYPTOMSG(msg)       \
  do {                                   \
...
SysciReturnCode Sysci_new(Sysci **new_sysci) {
  ...
  CryptoReturnCode crc =
      CryptoMsg_new((CryptoMsgData)sys_info.machine, strlen(sys_info.machine),
                    &(*new_sysci)->hardware_id);
  CRYPTO_GOTO_IF_ERROR(crc);
  crc = CryptoMsg_new((CryptoMsgData)sys_info.release, strlen(sys_info.release),
                      &(*new_sysci)->system_release);
  CRYPTO_GOTO_IF_ERROR(crc);
  crc = Crypto_digest_file(kSYSCI_EFI_FILE_PATH, &(*new_sysci)->efi_sha256);
  CRYPTO_GOTO_IF_ERROR(crc);
  crc = Crypto_digest_file(kSYSCI_SDW_TPM_PATH, &(*new_sysci)->sdw_tpm_sha256);
  CRYPTO_GOTO_IF_ERROR(crc);
...
void Sysci_free(Sysci *sysci) {
  if (sysci) {
    ...
    CryptoMsg_free(sysci->sdw_tpm_sha256);
...
SysciReturnCode Sysci_encrypt(const Sysci *sysci, CryptoMsg **encrypted_sysci) {
  ...
  CryptoMsg *encrypted_sdw_tpm_sha256 = NULL;
  CryptoMsg *encrypted_proxy_p_sha256 = NULL;
  (*encrypted_sysci) = NULL;

  CryptoReturnCode crc = Crypto_rsa_pub_file_encrypt(
      sysci->hardware_id, kRSA_PUB_FILE_PATH, &encrypted_hardware_id);
  CRYPTO_GOTO_IF_ERROR(crc);
  crc = Crypto_rsa_pub_file_encrypt(sysci->system_release, kRSA_PUB_FILE_PATH,
                                    &encrypted_system_release);
  CRYPTO_GOTO_IF_ERROR(crc);
  crc = Crypto_rsa_pub_file_encrypt(sysci->efi_sha256, kRSA_PUB_FILE_PATH,
                                    &encrypted_efi_sha256);
  CRYPTO_GOTO_IF_ERROR(crc);
  crc = Crypto_rsa_pub_file_encrypt(sysci->sdw_tpm_sha256, kRSA_PUB_FILE_PATH,
                                    &encrypted_sdw_tpm_sha256);
  CRYPTO_GOTO_IF_ERROR(crc);
  crc = Crypto_rsa_pub_file_encrypt(sysci->proxy_p_sha256, kRSA_PUB_FILE_PATH,
                                    &encrypted_proxy_p_sha256);
  CRYPTO_GOTO_IF_ERROR(crc);

  CryptoMsgDataLength encrypted_sysci_len =
      encrypted_hardware_id->data_len + encrypted_system_release->data_len +
      encrypted_efi_sha256->data_len + encrypted_sdw_tpm_sha256->data_len +
      encrypted_proxy_p_sha256->data_len;
  ...
  memcpy(off, encrypted_system_release->data,
         encrypted_system_release->data_len);
  ...
  memcpy(off, encrypted_sdw_tpm_sha256->data,
         encrypted_sdw_tpm_sha256->data_len);
  off += encrypted_sdw_tpm_sha256->data_len;
  CryptoMsg_free(encrypted_sdw_tpm_sha256);
  memcpy(off, encrypted_proxy_p_sha256->data,
         encrypted_proxy_p_sha256->data_len);
...
SysciReturnCode Sysci_decrypt(const CryptoMsg *encrypted_sysci, Sysci **sysci) {
  ...
  CryptoMsgData off = encrypted_sysci->data;
  CryptoReturnCode crc =
      CryptoMsg_new(off, kRSA_KEY_LENGTH, &encrypted_hardware_id);
  off += encrypted_hardware_id->data_len;
  crc = CryptoMsg_new(off, kRSA_KEY_LENGTH, &encrypted_system_release);
  off += encrypted_system_release->data_len;
  crc = CryptoMsg_new(off, kRSA_KEY_LENGTH, &encrypted_efi_sha256);
  off += encrypted_efi_sha256->data_len;
  crc = CryptoMsg_new(off, kRSA_KEY_LENGTH, &encrypted_sdw_tpm_sha256);
  off += encrypted_sdw_tpm_sha256->data_len;
  crc = CryptoMsg_new(off, kRSA_KEY_LENGTH, &encrypted_proxy_p_sha256);

  crc = Crypto_rsa_pri_file_decrypt(encrypted_hardware_id, kRSA_PRI_FILE_PATH,
                                    &(*sysci)->hardware_id);
  CryptoMsg_free(encrypted_hardware_id);
  Crypto_rsa_pri_file_decrypt(encrypted_system_release, kRSA_PRI_FILE_PATH,
                              &(*sysci)->system_release);
  CryptoMsg_free(encrypted_system_release);
  Crypto_rsa_pri_file_decrypt(encrypted_efi_sha256, kRSA_PRI_FILE_PATH,
                              &(*sysci)->efi_sha256);
  CryptoMsg_free(encrypted_efi_sha256);
  Crypto_rsa_pri_file_decrypt(encrypted_sdw_tpm_sha256, kRSA_PRI_FILE_PATH,
                              &(*sysci)->sdw_tpm_sha256);
  CryptoMsg_free(encrypted_sdw_tpm_sha256);
  Crypto_rsa_pri_file_decrypt(encrypted_proxy_p_sha256, kRSA_PRI_FILE_PATH,
                              &(*sysci)->proxy_p_sha256);
  CryptoMsg_free(encrypted_proxy_p_sha256);
  return SYSCI_RC_SUCCESS;
error:
  CryptoMsg_free(encrypted_hardware_id);
...
SysciReturnCode Sysci_to_json(const Sysci *sysci, char **sysci_json) {
  ...
  char *sdw_tpm_sha256_str = NULL;
  char *proxy_p_sha256_str = NULL;

  CryptoReturnCode crc =
      CryptoMsg_to_hexstr(sysci->hardware_id, &hardware_id_str);
  CRYPTO_GOTO_IF_ERROR(crc);
  ...
  crc = CryptoMsg_to_hexstr(sysci->sdw_tpm_sha256, &sdw_tpm_sha256_str);
  CRYPTO_GOTO_IF_ERROR(crc);
  cJSON *sdw_tpm_sha256 = cJSON_CreateString(sdw_tpm_sha256_str);
  free(sdw_tpm_sha256_str);
  sdw_tpm_sha256_str = NULL;
  ...
  cJSON_AddItemToObject(root, "sdw_tpm_sha256", sdw_tpm_sha256);
...
error:
  ...
  if (sdw_tpm_sha256_str) free(sdw_tpm_sha256_str);
...
SysciReturnCode Sysci_parse_from_json(const char *sysci_json, Sysci **sysci) {
  ...
  cJSON *sdw_tpm_sha256 = cJSON_GetObjectItem(root, "sdw_tpm_sha256");
  cJSON *proxy_p_sha256 = cJSON_GetObjectItem(root, "proxy_p_sha256");

  CryptoReturnCode crc = CryptoMsg_parse_from_hexstr(
      cJSON_GetStringValue(hardware_id), &(*sysci)->hardware_id);
  CRYPTO_GOTO_IF_ERROR(crc);
  crc = CryptoMsg_parse_from_hexstr(cJSON_GetStringValue(system_release),
                                    &(*sysci)->system_release);
  CRYPTO_GOTO_IF_ERROR(crc);
  crc = CryptoMsg_parse_from_hexstr(cJSON_GetStringValue(efi_sha256),
                                    &(*sysci)->efi_sha256);
  CRYPTO_GOTO_IF_ERROR(crc);
  crc = CryptoMsg_parse_from_hexstr(cJSON_GetStringValue(sdw_tpm_sha256),
                                    &(*sysci)->sdw_tpm_sha256);
  CRYPTO_GOTO_IF_ERROR(crc);
  crc = CryptoMsg_parse_from_hexstr(cJSON_GetStringValue(proxy_p_sha256),
                                    &(*sysci)->proxy_p_sha256);
  CRYPTO_GOTO_IF_ERROR(crc);
