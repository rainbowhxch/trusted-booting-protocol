...
static ReportReturnCode Report_empty_new(Report **report) {
  ...
  if ((*report) == NULL) return REPORT_RC_BAD_ALLOCATION;
  (*report)->id = NULL;
...
ReportReturnCode Report_new(const Sysci *sysci, const char *id,
  ...
  CryptoReturnCode crc =
      CryptoMsg_new((CryptoMsgData)id, strlen(id), &(*report)->id);
...
ReportReturnCode Report_sign(Report *report) {
  CryptoMsgDataLength need_sign_msg_len =
      report->id->data_len + report->timestamp->data_len +
...
ReportReturnCode Report_verify(const Report *report, int *verify_res) {
  int need_verify_msg_len = report->id->data_len + report->timestamp->data_len +
                            report->nonce->data_len +
                            report->encrypted_sysci->data_len;
  ...
  crc = Crypto_rsa_file_digest_verify(need_verify_msg, report->signature,
...
ReportReturnCode Report_parse_from_json(const char *report_json,
  ...
  cJSON *encrypted_sysci =
      cJSON_GetObjectItemCaseSensitive(root, "encrypted_sysci");
  cJSON *signature = cJSON_GetObjectItemCaseSensitive(root, "signature");

  CryptoReturnCode crc =
      CryptoMsg_parse_from_hexstr(cJSON_GetStringValue(id), &(*report)->id);
  CRYPTO_GOTO_IF_ERROR(crc);
  crc = CryptoMsg_parse_from_hexstr(cJSON_GetStringValue(timestamp),
                                    &(*report)->timestamp);
  CRYPTO_GOTO_IF_ERROR(crc);
  crc = CryptoMsg_parse_from_hexstr(cJSON_GetStringValue(nonce),
                                    &(*report)->nonce);
  CRYPTO_GOTO_IF_ERROR(crc);
  crc = CryptoMsg_parse_from_hexstr(cJSON_GetStringValue(encrypted_sysci),
                                    &(*report)->encrypted_sysci);
  CRYPTO_GOTO_IF_ERROR(crc);
