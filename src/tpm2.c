...
TSS2_RC tcti_proxy_transmit(TSS2_TCTI_CONTEXT *tctiContext, size_t command_size,
  ...
  rval =
      Tss2_Tcti_Transmit(tcti_proxy->tctiInner, command_size, command_buffer);
  if (rval != TSS2_RC_SUCCESS) {
...
TSS2_RC tcti_proxy_receive(TSS2_TCTI_CONTEXT *tctiContext,
                           size_t *response_size, uint8_t *response_buffer,
                           int32_t timeout) {
  TSS2_RC rval;
  TSS2_TCTI_CONTEXT_PROXY *tcti_proxy = tcti_proxy_cast(tctiContext);
  ...
  if (response_buffer != NULL) tcti_proxy->state = intercepting;
...
TSS2_RC tcti_proxy_initialize(TSS2_TCTI_CONTEXT *tctiContext,
                              size_t *contextSize,
...
static TSS2_RC create_policy_session(TSS2_SYS_CONTEXT *sys_ctx,
  ...
  rc = Tss2_Sys_StartAuthSession(sys_ctx, TPM2_RH_NULL, TPM2_RH_NULL, 0, &nonce,
                                 &salt, TPM2_SE_POLICY, &symmetric,
                                 TPM2_ALG_SHA1, handle, &nonce_tpm, 0);
...
TSS2_SYS_CONTEXT *sys_init_from_tcti_ctx(TSS2_TCTI_CONTEXT *tcti_ctx) {
  ...
  if (sys_ctx == NULL) {
    fprintf(stderr, "Failed to allocate 0x%zx bytes for the SYS context\n",
            size);
    return NULL;
  }
...
TSS2_SYS_CONTEXT *sys_init_from_opts(test_opts_t *options) {
  ...
  if (tcti_ctx == NULL) return NULL;
  sys_ctx = sys_init_from_tcti_ctx(tcti_ctx);
  if (sys_ctx == NULL) return NULL;
  return sys_ctx;
}
...
void esys_init_from_tcti_ctx(TSS2_TCTI_CONTEXT *tcti_context,
                             ESYS_CONTEXT **ctx) {
  TSS2_RC rc;
  TSS2_ABI_VERSION abiVersion = {TSSWG_INTEROP, TSS_SAPI_FIRST_FAMILY,
...
void esys_init_from_opts(test_opts_t *opts, ESYS_CONTEXT **esys_ctx,
...
TSS2_RC esys_teardown_full(ESYS_CONTEXT *esys_ctx,
                           TSS2_TCTI_CONTEXT *tcti_inner) {
  TSS2_TCTI_CONTEXT *tcti_context = NULL;
  TSS2_RC rc;
...
TSS2_RC TPM2_esys_context_init(ESYS_CONTEXT **esys_ctx,
  ...
  if ((*esys_ctx) == NULL) return TPM2_RC_BAD_CONTEXT;
  return TSS2_RC_SUCCESS;
}

TSS2_RC TPM2_esys_context_teardown(ESYS_CONTEXT *esys_ctx,
                                   TSS2_TCTI_CONTEXT *tcti_inner) {
  if (esys_ctx == NULL) return TPM2_RC_FAILURE;
  if (tcti_inner == NULL) return TPM2_RC_FAILURE;
...
TSS2_RC TPM2_esys_pcr_extend(ESYS_CONTEXT *ctx, Sysci *sysci,
  ...
  TPML_DIGEST_VALUES digests = {
      .count = 4,
      .digests = {
          {.hashAlg = TPM2_ALG_SHA256, .digest = {.sha256 = {0}}},
          {.hashAlg = TPM2_ALG_SHA256, .digest = {.sha256 = {0}}},
          {.hashAlg = TPM2_ALG_SHA256, .digest = {.sha256 = {0}}},
          {.hashAlg = TPM2_ALG_SHA256, .digest = {.sha256 = {0}}},
      }};
  memcpy(digests.digests[0].digest.sha256, sysci->hardware_id->data,
         sysci->hardware_id->data_len);
  memcpy(digests.digests[1].digest.sha256, sysci->system_release->data,
         sysci->system_release->data_len);
  memcpy(digests.digests[2].digest.sha256, sysci->efi_sha256->data,
         sysci->efi_sha256->data_len);
  memcpy(digests.digests[3].digest.sha256, sysci->sdw_tpm_sha256->data,
         sysci->sdw_tpm_sha256->data_len);
  TPML_DIGEST_VALUES remained_digests = {
      .count = 1,
      .digests = {
          {.hashAlg = TPM2_ALG_SHA256, .digest = {.sha256 = {0}}},
      }};
  memcpy(remained_digests.digests[0].digest.sha256, sysci->proxy_p_sha256->data,
         sysci->proxy_p_sha256->data_len);

  rc = Esys_PCR_Extend(ctx, pcrHandle_handle, ESYS_TR_PASSWORD, ESYS_TR_NONE,
                       ESYS_TR_NONE, &digests);
  TPM2_RETURN_IF_ERROR(rc);
  rc = Esys_PCR_Extend(ctx, pcrHandle_handle, ESYS_TR_PASSWORD, ESYS_TR_NONE,
                       ESYS_TR_NONE, &remained_digests);
  TPM2_RETURN_IF_ERROR(rc);

  TPML_PCR_SELECTION pcrSelectionIn = {.count = 2,
                                       .pcrSelections = {
                                           {
                                               .hash = TPM2_ALG_SHA1,
                                               .sizeofSelect = 3,
  ...
  CryptoReturnCode crc = CryptoMsg_new(pcrValues->digests[1].buffer,
                                       pcrValues->digests[1].size, pcr_digest);
  if (crc != CRYPTO_RC_SUCCESS) {
    return TPM2_RC_FAILURE;
  }

  rc = Esys_PCR_Reset(ctx, pcrHandle_handle, ESYS_TR_PASSWORD, ESYS_TR_NONE,
                      ESYS_TR_NONE);
  TPM2_RETURN_IF_ERROR(rc);
...
void TPM2_esys_nv_read(ESYS_CONTEXT *ctx, ESYS_TR nvHandle, CryptoMsg **data) {
  ...
  r = Esys_NV_Read(ctx, nvHandle, nvHandle, ESYS_TR_PASSWORD, ESYS_TR_NONE,
                   ESYS_TR_NONE, 32, 0, &nv_data);

  CryptoMsg_new(nv_data->buffer, nv_data->size, data);

  r = Esys_NV_UndefineSpace(ctx, ESYS_TR_RH_OWNER, nvHandle, ESYS_TR_PASSWORD,
...
void TPM2_esys_nv_write(ESYS_CONTEXT *ctx, ESYS_TR *nvHandle, CryptoMsg *data) {
  ...
  r = Esys_NV_Write(ctx, *nvHandle, *nvHandle, ESYS_TR_PASSWORD, ESYS_TR_NONE,
                    ESYS_TR_NONE, &nv_data, offset);
}
...
TSS2_RC TPM2_sys_context_init(TSS2_SYS_CONTEXT **sys_context) {
  ...
  if ((*sys_context) == NULL) return TPM2_RC_BAD_CONTEXT;
...
TSS2_RC TPM2_sys_context_teardown(TSS2_SYS_CONTEXT *sys_context) {
  if (sys_context == NULL) return TPM2_RC_FAILURE;
...
TSS2_RC TPM2_sys_nv_init(TSS2_SYS_CONTEXT *sys_ctx, TPMI_RH_NV_INDEX index) {
      ...
      .nvPublic =
          {
              ...
              .attributes = TPMA_NV_AUTHREAD | TPMA_NV_AUTHWRITE |
                            TPMA_NV_PLATFORMCREATE | TPMA_NV_WRITEDEFINE |
                            TPMA_NV_ORDERLY,
  ...
  rc = Tss2_Sys_NV_DefineSpace(sys_ctx, TPM2_RH_PLATFORM, &auth_cmd_null_pwd,
...
TSS2_RC TPM2_sys_nv_teardown(TSS2_SYS_CONTEXT *sys_ctx,
  ...
  rc = Tss2_Sys_NV_UndefineSpace(sys_ctx, TPM2_RH_PLATFORM, index,
                                 &auth_cmd_null_pwd, &auth_resp);
  TPM2_RETURN_IF_ERROR(rc);
...
TSS2_RC TPM2_sys_nv_write(TSS2_SYS_CONTEXT *sys_ctx, TPMI_RH_NV_INDEX nv_index,
  ...
  rc = TSS2_RETRY_EXP(Tss2_Sys_NV_Write(sys_ctx, nv_index, nv_index,
                                        &auth_cmd_null_pwd, &write_data, 0,
                                        &auth_resp));
  TPM2_RETURN_IF_ERROR(rc);
  return TSS2_RC_SUCCESS;
}
...
TSS2_RC TPM2_sys_nv_read(TSS2_SYS_CONTEXT *sys_ctx, TPMI_RH_NV_INDEX nv_index,
  ...
  rc = Tss2_Sys_NV_Read(sys_ctx, nv_index, nv_index, &auth_cmd_null_pwd, 32, 0,
                        &nv_buf, &auth_resp);
  TPM2_RETURN_IF_ERROR(rc);

  CryptoReturnCode crc = CryptoMsg_new(nv_buf.buffer, nv_buf.size, data);
  if (crc != CRYPTO_RC_SUCCESS) return TPM2_RC_FAILURE;
  return TSS2_RC_SUCCESS;
}
