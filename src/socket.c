...
inline static uint8_t *SocketMsg_get_magic_number_from_data(
    uint8_t *data, SocketMagicNumber *magic_number) {
  (*magic_number) = *((SocketMagicNumber *)data);
  return data += sizeof(SocketMagicNumber);
}

inline static uint8_t *SocketMsg_get_type_from_data(uint8_t *data,
                                                    SocketMsgType *type) {
  (*type) = *((SocketMsgType *)data);
  return data += sizeof(SocketMsgType);
}

inline static uint8_t *SocketMsg_get_data_len_from_data(uint8_t *data,
                                                        SocketDataLength *len) {
...
SocketReturnCode SocketMsg_new(const SocketMsgType type, const SocketData data,
  ...
  if ((*msg) == NULL) return SOCKET_RC_BAD_ALLOCATION;
...
SocketReturnCode Socket_udp_init(const uint16_t port, int *sockfd) {
  ...
  if ((*sockfd) == -1) return SOCKET_RC_SOCKET_FAILED;
  if (port != 0) {
    struct sockaddr_in addr;
...
SocketReturnCode Socket_send_to_peer(const int sockfd, const SA *peer_addr,
  ...
  if (rc != SOCKET_RC_SUCCESS) return rc;

  ssize_t write_len = sendto(sockfd, msg, SocketMsg_total_length(msg), 0,
...
SocketReturnCode Socket_read_from_peer(const int sockfd, SA *peer_addr,
  ...
  ssize_t read_len =
      recvfrom(sockfd, buf, kSOCKET_BUFFER_SIZE, 0, peer_addr, peer_addr_len);
