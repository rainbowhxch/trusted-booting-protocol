...
static int send_report_to_proxy_v(Sysci *sysci, const char *id) {
	...
	if (kPRE_NONCE == NULL) {
		CryptoReturnCode crc = CryptoMsg_new(report->nonce->data, report->nonce->data_len, &kPRE_NONCE);
...
static int verify_verifyResponse(VerifyResponse *verify_response) {
	int verify_res;
	VerifyResponseReturnCode vrc = VerifyResponse_verify(verify_response, &verify_res);
	VERIFY_RESPONSE_WRITE_LOG_AND_GOTO_IF_ERROR(kLOG_FD, vrc, verify_verifyResponse_error);
	return verify_res;
...
static void parse_proxy_v_msg_loop_pre() {
...
static void parse_proxy_v_msg_loop(const char *id) {
	if (!requre_sysci_and_send_report_once(id))
		exit(EXIT_FAILURE);
	SocketMsg *sock_msg = NULL;
	while (1) {
		SocketReturnCode src = Socket_read_from_peer(kSOCK_FD,
												     (SA *)&kPROXY_V_ADDR,
							                         &kPROXY_V_ADDR_LEN,
													 &sock_msg);
			...
			case SOCKET_MT_VERIFY_RESULT:
				{
					...
					VerifyResponseReturnCode vrc = VerifyResponse_parse_from_json((char *)sock_msg->data, &verify_response);
					VERIFY_RESPONSE_WRITE_LOG_AND_GOTO_IF_ERROR(kLOG_FD, vrc, parse_verifyResponse_error);
					if (!verify_verifyResponse(verify_response))
						verify_failed_handle();
					if (!verify_nonce(verify_response->nonce))
						verify_failed_handle();
