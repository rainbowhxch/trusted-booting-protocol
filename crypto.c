...
CryptoReturnCode CryptoMsg_parse_from_hexstr(const char* hexstr, CryptoMsg **msg) {
    ...
    for (size_t i = 0, j = 0; j < final_len; i += 2, ++j)
        (*msg)->data[j] = (hexstr[i] % 32 + 9) % 25 * 16 + (hexstr[i+1] % 32 + 9) % 25;
error:
    return rc;
}
...
CryptoReturnCode Crypto_digest_message(const CryptoMsg *msg, CryptoMsg **digest) {
	EVP_MD_CTX *mdctx;
...
CryptoReturnCode Crypto_rsa_pub_file_encrypt(const CryptoMsg *msg, const char *pub_key_path, CryptoMsg **encrypted_msg) {
	...
	if (!fd)
		return CRYPTO_RC_OPEN_FILE_FAILED;
...
CryptoReturnCode Crypto_rsa_pri_file_decrypt(const CryptoMsg *msg, const char *pri_key_path, CryptoMsg **decrypted_msg) {
...
CryptoReturnCode Crypto_rsa_pub_key_encrypt(const CryptoMsg *msg, EVP_PKEY *pkey, CryptoMsg **encrypted_msg) {
	...
	if (EVP_PKEY_encrypt(ctx, NULL, &encrypted_msg_len, msg->data, msg->data_len) <= 0)
		CRYPTO_EVP_PKEY_CTX_FREE_AND_RETURN(ctx, CRYPTO_RC_EVP_FAILED);
	CryptoReturnCode rc = CryptoMsg_new_with_length(encrypted_msg_len, encrypted_msg);
	CRYPTO_GOTO_IF_ERROR(rc);
	if (EVP_PKEY_encrypt(ctx, (*encrypted_msg)->data, &(*encrypted_msg)->data_len, msg->data, msg->data_len) <= 0)
		CRYPTO_EVP_PKEY_CTX_FREE_AND_RETURN(ctx, CRYPTO_RC_EVP_FAILED);
error:
...
CryptoReturnCode Crypto_rsa_pri_key_decrypt(const CryptoMsg *msg, EVP_PKEY *pkey, CryptoMsg **decrypted_msg) {
	...
	if (EVP_PKEY_decrypt(ctx, NULL, &decrypted_msg_len, msg->data, msg->data_len) <= 0)
		CRYPTO_EVP_PKEY_CTX_FREE_AND_RETURN(ctx, CRYPTO_RC_EVP_FAILED);
	CryptoReturnCode rc = CryptoMsg_new_with_length(decrypted_msg_len, decrypted_msg);
	CRYPTO_GOTO_IF_ERROR(rc);
	if (EVP_PKEY_decrypt(ctx, (*decrypted_msg)->data, &(*decrypted_msg)->data_len, msg->data, msg->data_len) <= 0)
		CRYPTO_EVP_PKEY_CTX_FREE_AND_RETURN(ctx, CRYPTO_RC_EVP_FAILED);
error:
	CRYPTO_EVP_PKEY_CTX_FREE_AND_RETURN(ctx, rc);
}
...
CryptoReturnCode Crypto_rsa_file_digest_sign(const CryptoMsg *digest, const char *pri_key_path, CryptoMsg **signed_digest) {
	...
	if(!mdctx)
		CRYPTO_EVP_PKEY_FREE_AND_RETURN(pkey, CRYPTO_RC_EVP_FAILED);
	...
	* signature. Length is returned in slen */
	...
	if(1 != EVP_DigestSignFinal(mdctx, NULL, &signed_digest_len))
		CRYPTO_EVP_MD_PKEY_FREE_AND_RETURN(mdctx, pkey, CRYPTO_RC_EVP_FAILED);
	CryptoReturnCode rc = CryptoMsg_new_with_length(signed_digest_len, signed_digest);
	CRYPTO_GOTO_IF_ERROR(rc);
	/* Obtain the signature */
	if(1 != EVP_DigestSignFinal(mdctx, (*signed_digest)->data, &(*signed_digest)->data_len))
		CRYPTO_EVP_MD_PKEY_FREE_AND_RETURN(mdctx, pkey, CRYPTO_RC_EVP_FAILED);
